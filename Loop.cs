...
namespace TinyMUD
{
	public static class Loop
	{
		...
		private static readonly List<Action<string, Event>> events_action = new List<Action<string, Event>>();
		...
		public static void Broadcast(string msg, Event evt)
		{
			...
			if (events.TryGetValue(msg, out actions))
			{
				...
				{
					events_action.Add(enumerator.Current.Value);
				}
				enumerator.Dispose();
				for (int i = 0; i < events_action.Count; ++i)
				{
					try
					{
						events_action[i](msg, evt);
					}
					catch (Exception e)
					{
						errfn(e);
		...
		public static void Catch(Action<Exception> fn)
		{
			if (fn == null)
			{
				errfn = e => { };
